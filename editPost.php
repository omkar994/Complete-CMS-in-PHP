...
<?php
    ...
    if(isset($_POST["submit"])){
        ...
        elseif(empty($image)){
            $_SESSION["errorMessage"]="* Please upload the IMAGE";
        }

        else{
            global $ConnectingDB;
            
            $sql="update cms.posts set datetime=:datetimE ,title=:titlE ,category=:categorY, author=:authoR, image=:imagE, post=:posT where id='$searchQueryID'";
            ...
            if($Execute){
                ...
                echo '<script>window.open("blog.php?id=record updated successfully","_self");
				</script>';
            }
            else{
    ...
    <body>
        ...
        <header class="bg-dark text-white py-3">
            <div class="cointainer">
                <div class="row">
                    <div class="col-md-12">
                      <h1 style="padding-left: 50px;"><i class="fas fa-edit" style="padding-left: 5px;"></i>Edit Post</h1>
                   </div>
        ...
        <section class="container py-2 mb-4">
            <div class="row">
                ...
                $fetchData="select * from cms.posts where id='$searchQueryID'";
                $stmt2=$ConnectingDB->query($fetchData);

                    while($DataRows=$stmt2->fetch()){
                        $eDateTime=$DataRows["datetime"];
                        $eTitle=$DataRows["title"];
                        $eCategory=$DataRows["category"];
                        $eAuthor=$DataRows["author"];
                        $ePost=$DataRows["post"];
                    }

                ?>
                <form action="editPost.php?id=<?php echo $searchQueryID?>" method="post" enctype="multipart/form-data">
                    <div class="card  bg-secondary text-light mb-3">
                        
                        <div class="card-body bg-dark">
                            <div class="form-group">
                                <label for="title"><span class="fieldInfo">Post Title: </span></label>
                                <input  class="form-control mb-3" type="text" id="title" name="postTitle" value="<?php echo $eTitle?>">
                            </div>
                            <div class="form-group">
                            ...
                            <textarea class="form-control" name="postDescription" id="post" cols="30" rows="8" value="ePost"><?php echo $ePost?></textarea>
                            </div>
